...
Zin UI zip(UI*p,UI ba,UI idx,U b){P(!b,ba)UI n=_(popcountll)(b),nxb=ba+n;
 N(8,z(i))Z(n>8,N(8,z(i+8)));Z(n>16,ba+=16;do{z(0)ba++;}W(b));R nxb;} //fixme n>16 dead branch?
...
U ld(const S s,U len,CSV*r){span in;U intl_idx,sep,trm,qt_mask,f_sep,idx=0,base=0,in_qt=0;UI*base_ptr=r->i;len-=len<64?0:64;
 if(BN&&len>64*BN){U fields[BN];for(;idx<len-64*BN+1;idx+=64*BN){ //eliminated if BN is zero
  N(BN,intl_idx=64*i+idx;_(prefetch)(s+intl_idx+128);in=vld(s+intl_idx);mask(fields[i]))
  N(BN,intl_idx=64*i+idx;base=zip(base_ptr,base,intl_idx,fields[i]))}}
 for(;idx<len;idx+=64){_(prefetch)(s+idx+128);in=vld(s+idx);mask(f_sep)base=zip(base_ptr,base,idx,f_sep);}R r->n=base;}
