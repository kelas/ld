...
Zin U fqm(span x,U*in_q){U qbts=cmi(x,'"');U m=vmull_p64(-1ULL,qbts);R SHR(m);}
#else
Zin U cmi(span z,G m){__m256i b=vset1(m);U x=(I)mvmask(cmpeq(z.lo,b)),y=mvmask(cmpeq(z.hi,b));R(y<<32)|x;}
Zin U fqm(span x,U*in_q){U qbts=cmi(x,'"');U m=_mm_cvtsi128_si64(_mm_clmulepi64_si128(_mm_set_epi64x(0ULL,qbts),_mm_set1_epi8(0xFF),0));R SHR(m);}
#endif
