...
static inline U _umul128(U ab, U cd, U *hi) {
...
value128 full_multiplication(U value1, U value2) {
  value128 answer;
#ifdef _M_ARM64
...
static I leading_zeroes(U n) {R __builtin_clzll(n);}
...
static I is_Integer(char c) {R (c >= '0' && c <= '9');} // this gets compiled to (G)(c - '0') <= 9 on all decent compilers
...
const S parse_number(S p, F *outDouble) {
  const S pInit = p;
  I found_minus = (*p == '-');
  I negative = 0;
  if (found_minus) {
  ...
  I digit_count = (I)(p - start_digits - 1); // used later to guard against overflows
