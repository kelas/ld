...
typedef struct value128 {uint64_t low; uint64_t high;}value128;
...
//#ifdef __SIZEOF_INT128__  __uint128_t r = ((__uint128_t)value1) * value2;answer.low = uint64_t(r);nswer.high = uint64_t(r >> 64); // this is what we have on most 32-bit systems
...
static int leading_zeroes(uint64_t input_num) {return __builtin_clzll(input_num);}

// Precomputed powers of ten from 10^0 to 10^22. These
// can be represented exactly using the double type.
static const double power_of_ten[] = { 1e0,  1e1,  1e2,  1e3,  1e4,  1e5,  1e6,  1e7,  1e8,  1e9,  1e10, 1e11, 1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20, 1e21, 1e22};
static inline int is_integer(char c) {return (c >= '0' && c <= '9');} // this gets compiled to (uint8_t)(c - '0') <= 9 on all decent compilers
...
really_inline static double compute_float_64(int64_t power, uint64_t i, int negative, int*success) {
...
const char * parse_number(const char *p, double *outDouble) {
  ...
  if (*p == '0'||*p == '.') {
    //if (0==*p++&&is_integer(*p)) {return nullptr;} //kelas: in js(on), 0 cannot be followed by an integer and cannot start with dot. we fix that.
    i = 0;
  } else {
